...
struct PowerballRemainingTimeTriggerNode : public BehaviorNode {
  PowerballRemainingTimeTriggerNode(float time) : time(time) {}
  ...
  ExecuteResult Execute(ExecuteContext& ctx) override {
    ...
    if (soccer.carry_timer < time) return ExecuteResult::Success;
