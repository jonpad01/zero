...
Connection::TickResult Connection::Tick() {
  ...
  while (true) {
    ...
    // Buffer must be reset after packet sequencer runs so the read pointer is
    // reset.
    buffer.Reset();
...
void Connection::ProcessPacket(u8* pkt, size_t size) {
  ...
  last_packet_tick = GetCurrentTick();
  ...
  if (type_byte == 0x00) {  // Core packet
    ...
    switch (type) {
      ...
      case ProtocolCore::SyncTimeResponse: {
        ...
        size_t current_index = sync_index++ % ZERO_ARRAY_SIZE(sync_history);
        ...
        if (history_count > ZERO_ARRAY_SIZE(sync_history)) {
          history_count = ZERO_ARRAY_SIZE(sync_history);
        }
...
#pragma pack(pop)
    ...
    switch (type) {
      ...
      case ProtocolS2C::MapInformation: {
        ...
        requester.Request(filename, 0, map.compressed_size, map.checksum, true, zero::OnDownloadComplete, this);
      } break;
...
ConnectResult Connection::Connect(const char* ip, u16 port) {
  ...
  this->last_packet_tick = GetCurrentTick();
