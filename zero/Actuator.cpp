...
// There are 40 possible heading orientations. This is half way in between the two points.
const float FREE_PLAY = 79.0f / 80.0f;
...
void Actuator::Update(Game& game, InputState& input, const Vector2f& force, float rotation) {
  ...
  if (heading.Dot(steering_direction) < FREE_PLAY) {
